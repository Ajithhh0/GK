...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, phone, organisation, interest, message } = body
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `Dream Project Enquiry — ${interest || 'General'} — ${name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #1B5E20 0%, #2E7D32 100%); padding: 32px; text-align: center; border-radius: 8px 8px 0 0;">
            ...
            <p style="color: #a5d6a7; margin: 8px 0 0;">Dream Project — Old Age Home Enquiry</p>
          </div>
          <div style="background: #f9f9f9; padding: 32px; border: 1px solid #e0e0e0; border-radius: 0 0 8px 8px;">
            <h2 style="color: #1B5E20; font-size: 18px; margin-top: 0;">New Enquiry Received</h2>
